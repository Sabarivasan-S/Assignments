...
app.post('/login',async (req,res)=>{
  ...
  const refreshToken = await jwt.sign(id, process.env.REFRESH_TOKEN_SECRET);
  const accessToken=await jwt.sign(id, process.env.ACCESS_TOKEN_SECRET,{ expiresIn: '15m' });
...
app.put('/update',authenticateToken,async (req,res)=>{
  ...
  await users.update(req.body,{where:{userid:req.userid}});
